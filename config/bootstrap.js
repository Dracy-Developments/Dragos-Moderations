...
*/
  ...
  Discord.Structures.extend('Guild', Guild => {
    class CoolGuild extends Guild {
      ...
      // Per-guild rules
      get rules () {
        return Caches.get('rules').collection.filter((record) => record.guildID === this.id);
      }
    }
...
*/
  ...
  global[ 'Client' ] = new DiscordClient(sails.config.custom.discord.clientOptions);
  ...
  */
  ...
  await sails.models.schedules.findOrCreate({ uid: 'SYS-MINUTELY' }, { uid: 'SYS-MINUTELY', task: 'sysMinutely', cron: '0 * * * * *' });
};
