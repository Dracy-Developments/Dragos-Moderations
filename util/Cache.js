...
class CacheContainer {
  ...
  find(values, create = true) {
    if (!this.initialized) throw new Error("Model is not yet initialized.");
    var criteria = {};
  ...
  set(values, criteria) {
    sails.log.verbose(
      `${this.model}=>set(${JSON.stringify(
        values
      )}): set called (${JSON.stringify(criteria)})`
    );
    ...
    if (!this.initialized) {
      ...
      var record = this.collection.find(
        (record) => this.filterByUnique(record, values) && !isNaN(record.id)
      );
      if (record) {
        sails.log.verbose(
          `${this.model}=>set(${JSON.stringify(
            values
          )}): Record found. Updating cache.`
        );
        ...
        sails.log.verbose(
          `${this.model}=>set(${JSON.stringify(values)}): Record not found.`
        );
  ...
  delete(id) {
    ...
    sails.log.verbose(
      `${this.model}=>delete(${id}): Deleting record from cache.`
    );
  ...
  _create(values, criteria) {
    sails.log.verbose(
      `${this.model}=>_create(${JSON.stringify(values)}): Called.`
    );
    ...
    sails.log.verbose(
      `${this.model}=>_create(${JSON.stringify(values)}): Temp ID is ${key}.`
    );
  ...
  queueSync(key) {
    ...
    if (!this.running) {
      ...
      if (this.dbQueue.indexOf(key) !== -1) {
        sails.log.verbose(
          `${this.model}=>queueSync(${key}): DB operations running, but this record is already queued for sync.`
        );
      } else {
        sails.log.verbose(
          `${this.model}=>queueSync(${key}): DB operations running; queued sync.`
        );
  ...
  _sync(key) {
    ...
    var nextTask = () => {
      ...
      if (this.dbQueue.length > 0) {
        ...
        sails.log.verbose(
          `${this.model}=>_sync(${key})=>nextTask(): Nothing more to do; DB operations finished.`
        );
        this.running = false;
      }
    };
    ...
    if (key && isNaN(key) && key.startsWith("F_")) {
      var record = this.collection.find(
        (record) => key === this.generateKeyByFields(record)
      );
      if (record) {
        sails.log.verbose(
          `${this.model}=>_sync(${key}): Real id found: ${record.id}.`
        );
        key = record.id;
      } else {
        sails.log.verbose(
          `${this.model}=>_sync(${JSON.stringify(values)}): Real id NOT found.`
        );
      }
    }
    ...
    switch (action) {
        ...
        sails.models[this.model]
          ...
          .exec((err, recordx) => {
            if (!err) {
              ...
              this.collection.set(
                recordx.id,
                Object.assign(record, { id: recordx.id })
              );
              sails.log.verbose(
                `${this.model}=>_sync(${key}): Updated cache temp id to permanent id ${recordx.id}.`
              );
            } else {
      ...
      case "destroy":
        sails.log.verbose(
          `${this.model}=>_sync(${key}): Destroying DB record.`
        );
        sails.models[this.model]
          .destroy({ id: key })
          .fetch()
    ...
    if (Object.prototype.hasOwnProperty.call(attributes, key)) {
      temp[key] =
        typeof defaults[key] !== "undefined"
          ? defaults[key]
          : typeof attributes[key].defaultsTo !== "undefined"
          ? attributes[key].defaultsTo
          : attributes[key].autoCreatedAt || attributes[key].autoUpdatedAt
          ? undefined
          : null;
    }
  }
