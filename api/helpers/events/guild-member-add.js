module.exports = {
  ...
  fn: async function (inputs) {
    ...
    // TODO
    /*
    const verifiedRole = inputs.member.guild.roles.resolve(
      inputs.member.guild.settings.verifiedRole
    );
    const muteRole = inputs.member.guild.roles.resolve(
    ...
    */
    ...
    if (
      muteRole &&
      (inputs.member.settings.muted ||
        inputs.member.roles.cache.has(muteRole.id))
    ) {
      inputs.member.settings.update(`muted`, true, inputs.member.guild);
      inputs.member.roles.set(
        [inputs.member.guild.settings.muteRole],
      ...
      await sails.helpers.guild.send(
        "modLogChannel",
        inputs.member.guild,
        `:mute: The member <@!${inputs.member.user.id}> had a mute on their account and was re-muted upon entering the guild. Check to be sure they were not trying to mute evade.`
      );
    } else {
      // Re-assign saved roles
      if (
        inputs.member.settings.verified &&
        inputs.member.settings.roles.length > 0
      ) {
        inputs.member.roles
          .set(inputs.member.settings.roles, `Re-assigning roles`)
          .then(async () => {
            // Verify the member if we are not in raid mitigation level 2+
            if (
              inputs.member.guild.settings.raidMitigation < 2 &&
              verifiedRole
            ) {
            ...
            await sails.helpers.guild.send(
              "generalChannel",
              inputs.member.guild,
              `**Welcome back** <@${inputs.member.id}>! I see you have been here before. I remembered the roles you had and all of your profile information / rewards. Please be sure to re-read the rules and information channels as things may have changed.`
            );
        ...
        if (inputs.member.guild.settings.raidMitigation < 2 && verifiedRole) {
          ...
          await sails.helpers.guild.send(
            "unverifiedChannel",
            inputs.member.guild,
            `**Welcome** <@${inputs.member.id}>! Please stand by for a short while, hopefully less than a couple hours; you had already previously passed verification, but due to an ongoing raid, I cannot let you have full guild access until the raid ends.`
          );
        }
      } else if (verifiedRole) {
        await sails.helpers.guild.send(
          "unverifiedChannel",
          inputs.member.guild,
          `**Welcome new member** <@${inputs.member.id}>! You are currently unverified and may not have access to the entire guild. Please check the information channels for instructions on how to get verified. You may talk with staff and other unverified members here in the meantime.`
        );
      } else {
        await sails.helpers.guild.send(
          "generalChannel",
          inputs.member.guild,
          `**Welcome new member** <@${inputs.member.id}>! Please check out the information channels for the rules and more information about us. Enjoy your stay!`
        );
      }
    }
    */
    ...
    if (
      moment()
        .subtract(7, "days")
        .isBefore(moment(inputs.member.user.createdAt))
    ) {
      await sails.helpers.guild.send(
        "flagLogChannel",
        inputs.member.guild,
        `:clock7: Member <@${inputs.member.user.id}> (${inputs.member.user.id}) just joined the guild but their user account is less than 7 days old. Trolls often create new accounts, so keep an eye on them.`
      );
    }
    */
    ...
    if (inputs.member.moderation.length > 0) {
      ...
      if (logs.length > 0) {
        await sails.helpers.guild.send(
          "flagLogChannel",
          inputs.member.guild,
          `:police_officer: Member <@${inputs.member.user.id}> (${inputs.member.user.id}) just re-joined the guild. Keep an eye on them because they have ${logs.length} discipline records on their account. (they have ${inputs.member.HP} HP).`
        );
      }
    }
    */
